 ...
 */
export function normalizeWsScheme(origin: string): string {
  const sep = origin.indexOf('://');
  if (sep < 0) return origin;
  const scheme = origin.slice(0, sep).toLowerCase();
  if (scheme === 'wss') return 'https://' + origin.slice(sep + 3);
  if (scheme === 'ws') return 'http://' + origin.slice(sep + 3);
  return origin;
}

export async function resolveLoopbackOrigin(origin: string): Promise<ResolvedOrigin> {
...
export interface ResolvedOrigin {
  ...
  resolvedIp: string;
  family: 4 | 6;
  /** Origin string with the IP literal substituted for the hostname. */
  ipUrl: string;
}
...
export function isLoopbackIP(ip: string): boolean {
  if (LOOPBACK_V4_RE.test(ip)) return true;
...
function parseOriginStrict(origin: string): ParsedOrigin {
  const m = ORIGIN_RE.exec(origin);
  if (!m) throw new Error(`invalid origin: ${origin}`);
  ...
  if (authority.startsWith('[')) {
    ...
    if (closeIdx < 0) throw new Error(`invalid origin: ${origin}`);
    host = authority.slice(1, closeIdx);
    const rest = authority.slice(closeIdx + 1);
    if (!rest.startsWith(':')) throw new Error(`invalid origin: ${origin}`);
    port = rest.slice(1);
  } else {
    const colonIdx = authority.lastIndexOf(':');
    if (colonIdx < 0) throw new Error(`invalid origin: ${origin} (port required)`);
    host = authority.slice(0, colonIdx);
    port = authority.slice(colonIdx + 1);
  }
  if (!port || !/^\d+$/.test(port)) throw new Error(`invalid origin: ${origin} (port required)`);
  return {scheme: scheme as 'http' | 'https', host, port};
}
