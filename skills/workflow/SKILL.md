...
Route session URLs to the right workflow. Auto-detects environment, infers intent, delegates.
...
**Session Analysis, UX Review, Reproduce:** MUST run as Task subagent. Review tool responses are large and will burn through the main context window. Give the subagent: session coordinates, MCP tool prefix, the user's original message.

**Bug Fix:** Do NOT run as subagent. Bug Fix needs the main context for code editing. Invoke `subtext:bug-fix-workflow` directly, passing session URL and MCP tool prefix.
...
- `subtext:shared` — MCP prefix conventions
