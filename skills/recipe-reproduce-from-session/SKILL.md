...
3. **Set up tunnel if localhost**: Call `live-connect({ url })` — if it returns `tunnel_required`, call `tunnel-connect({ relayUrl, target })` on subtext-tunnel, then retry `live-connect({ url, connection_id })` (see `subtext:tunnel`).
