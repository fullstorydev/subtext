...
> **PREREQUISITE — Read inline before any other action:** Read skills `subtext:session`, `subtext:shared`. For local dev URLs and build commands, also read `local-dev`. Do not use the Skill tool — read the files directly.
...
- Otherwise → delegate to subagent running `subtext:session-analysis-workflow`. Give it: session coordinates, bug description, MCP prefix.
...
- Build the UI (consult `local-dev`)
