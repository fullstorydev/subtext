...
> **PREREQUISITE — Read inline before any other action:** Read skills `subtext:session`, `subtext:live`, `subtext:sightmap`, `subtext:shared`, `subtext:tunnel`. Also read `local-dev` for local dev URLs and build commands. Live browser tools are provided by the subtext plugin. Do not use the Skill tool — read the files directly.
...
Also needs: local dev URL (from `local-dev` skill) and login instructions if auth required.
...
- Authentication required → follow login instructions from `local-dev`
