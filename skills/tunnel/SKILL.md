...
| `tunnel-connect` | subtext-tunnel | Connect local server to relay, returns `connectionId` |
...
2. Call `tunnel-connect` on the **subtext-tunnel** MCP server with `relayUrl` and `target` (no `connectionId`)
...
1. Call `live-tunnel` on the **subtext** MCP server → returns `relayUrl`
2. Call `tunnel-connect` on the **subtext-tunnel** MCP server with `relayUrl`, `connectionId` (from the existing `live-connect`), and `target`
