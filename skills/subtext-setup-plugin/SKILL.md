...
- **Skills** — `subtext-search` (find sessions by what happened in them — navigate/network/custom signals), `subtext-review` (structured summary of a session), `subtext-session` (the `review-*` tool catalog), `subtext-privacy` (PII detection + element-block/URL/network privacy rules), `subtext-telemetry` (workflow milestone logging).
