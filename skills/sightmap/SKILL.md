...
- **components** (optional): Array of component definitions
  ...
  - **source** (optional): Relative path to the source file implementing this component. Used by agents to navigate to source code.
  - **description** (optional): Brief description of the component's purpose. Used by agents for context when investigating issues.
...
When sightmap definitions are loaded, snapshots automatically annotate matched elements with semantic names and `[src: path]` annotations. The `source:` field gives you exact file paths — use those directly instead of grepping for component names or CSS classes.
