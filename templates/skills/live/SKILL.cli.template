...
- **Default to {{tool "live-view-snapshot"}} for all page observation.** It carries sightmap context (component names, view names, `[src: ...]` annotations) and provides the component UIDs needed by `act-*` commands. Always call it before any interaction sequence.
- **Use {{tool "live-view-screenshot"}} only for visual evidence** — before/after comparisons, layout debugging, or screenshots to embed in PRs. The command returns the image inline by default so you can verify framing. Pass `--upload` only when you need a hosted URL for a PR or comment attachment.
...
1. Run {{tool "live-tunnel"}} — allocates a browser connection and returns `relayUrl` + `connectionId`
2. Run {{tool "tunnel-connect"}} with `relayUrl` and `allowedOrigins` (one or more local origins the tunnel may serve)
3. Run {{tool "live-view-new"}} with the `connection_id` and localhost URL
