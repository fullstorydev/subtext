...
If the project has `.sightmap/` definitions, upload them after getting the URL and **before** proceeding (before {{tool "live-view-new"}} for the tunnel-first flow):
...
The upload uses a single-use token embedded in the URL — no additional auth is needed. Do NOT pass the `sightmap` parameter directly to {{tool "live-connect"}}.
