...
| Tool | Description |
|------|-------------|
| {{tool "review-open"}} | Open a session for analysis. Returns event summaries, metadata, timestamps. |
| {{tool "review-view"}} | Capture UI state at a timestamp — component tree + screenshot. Pass `component_id` to clip to a specific element's bounding box; optional `expand_pct` (0–100) grows the clip rect outward for surrounding context, clamped to the viewport. |
| {{tool "review-inspect"}} | Component tree with full CSS selectors — for sightmap authoring only, not general use |
| {{tool "review-diff"}} | Compare UI state between two timestamps |
| {{tool "review-close"}} | Close the session and free resources |
...
{{tool "review-open"}} accepts five mutually-exclusive identifiers. Pick the one that matches what you actually have on hand — they're all first-class:

- `trace_id` — the 12-char base62 id from a prior {{tool "live-connect"}} or {{tool "review-open"}} response. Resolves to the underlying device:session via the trace store. Use this when you're staying inside the trace flow (e.g., re-opening for a follow-on review, or pivoting between live and review surfaces).
...
- Event summaries from {{tool "review-open"}} are cheap. {{tool "review-view"}} is expensive. Start with summaries.
...
- {{tool "review-diff"}} between before/after is the most revealing tool — it shows exactly what changed.
- Console errors and network failures in event summaries are highest-signal starting points.
- Component names in {{tool "review-view"}}/{{tool "review-diff"}} output include `[src: ...]` annotations — use these to find source files directly.
