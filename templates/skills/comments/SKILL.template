...
> **Note:** To attach a screenshot, first capture one via {{tool "live-view-screenshot"}}{{if ne .Target "cli"}} or {{tool "review-view"}}{{end}}, then pass the returned URL as `screenshot_url` **verbatim** — the signed query string (`?Expires=…&GoogleAccessId=…&Signature=…`) is the credential. Stripping it returns 403 from GCS and the image won't render.
